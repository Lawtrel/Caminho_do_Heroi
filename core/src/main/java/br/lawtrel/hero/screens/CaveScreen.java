...
public class CaveScreen extends ScreenAdapter {
    ...
    private final String MAP_ID = "maps/cave.tmx";
    ...
    public CaveScreen(Hero game, MapManager mapManager){
        this.game = game;
    ...
    public void show(){
        ...
        if (player == null) {
            Gdx.app.error("CaveScreen", "Jogador nulo ao mostrar a tela!");
            return;
        }
                ...
                if (player.getBounds().overlaps(((RectangleMapObject) object).getRectangle())) {
                    ...
                    if (targetMap != null) {
                        ...
                        switch (targetMap.toLowerCase()) {
                            case "castle":
                                mapManager.changeMap(MapManager.MapType.CASTLE);
                                return;
                            case "world":
    ...
    private Vector2 findSpawnPoint(TiledMap map){
        ...
        Gdx.app.log("CaveScreen", "Objeto 'spawnPoint' não encontrado na camada 'Spawn'. Usando valor padrão.");
